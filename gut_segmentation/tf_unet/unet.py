...
from gut_segmentation.tf_unet import util
from gut_segmentation.tf_unet.layers import (weight_variable, weight_variable_devonc, bias_variable,
...
def create_conv_net(x, keep_prob, channels, n_class, layers=3, features_root=16, filter_size=3, pool_size=2, summaries=True):
    ...
    for layer in range(0, layers):
        features = 2**layer*features_root # Number of features per layer features_root, 2*features_root, 4*features_root...
        stddev = np.sqrt(2 / (filter_size**2 * features))  # Determining initial spread of randomized weights
        if layer == 0:
        ...
        conv1 = conv2d(in_node, w1, keep_prob)  # It appears that there are always 2 conv_layers per layer.
        tmp_h_conv = tf.nn.leaky_relu(conv1 + b1)
        conv2 = conv2d(tmp_h_conv, w2, keep_prob)
        dw_h_convs[layer] = tf.nn.leaky_relu(conv2 + b2)  # These are saved so that they can be added to uplayers
        ...
        size -= 4
        if layer < layers-1:    # Pool the layer if not the last, update in_node
    ...
    # up layers
    for layer in range(layers-2, -1, -1):  # builds the up layers backwards
        features = 2**(layer+1)*features_root
        stddev = np.sqrt(2 / (filter_size**2 * features))
        
        wd = weight_variable_devonc([pool_size, pool_size, features//2, features], stddev)
        bd = bias_variable([features//2])
        h_deconv = tf.nn.leaky_relu(deconv2d(in_node, wd, pool_size) + bd)  # Deconvolve
        h_deconv_concat = crop_and_concat(dw_h_convs[layer], h_deconv)
        deconv[layer] = h_deconv_concat  # combined down layers output and up layers input.
...
class Trainer(object):
    ...
    def _get_optimizer(self, training_iters, global_step):
        if self.optimizer == "momentum":
            ...
            decay_rate = self.opt_kwargs.pop("decay_rate", 0.97)
