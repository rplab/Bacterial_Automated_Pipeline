...
def quit_me():
    if len(aggregate_mask_3D)>0:
        np.savez_compressed(amo.save_mask_loc(ImgFiles) + 'region_'+str(region) +'_'+str(color) + '_aggregate_mask', np.array(aggregate_mask_3D),cur)
    window.quit()
    window.destroy()
    print('quit')
...
window.bind("<Return>", update_value)
