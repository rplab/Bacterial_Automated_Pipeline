...
def read_in_images(directory_loc, label_string='_gutmask', size2=256, size1=200, test_size=0.0):
    files = glob(directory_loc + '/*.tif', recursive=True)
    sort_nicely(files)
    mask_files = [item for item in files if label_string in item][4000:8000]
    data_files = [re.sub('\_mask.tif$', '.tif', item) for item in mask_files][4000:8000]  #  insert mask_string ref ***
    # mask_files = []
    # data_files = []
